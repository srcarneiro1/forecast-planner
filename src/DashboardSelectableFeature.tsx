...
export default function DashboardSelectableFeature(){
  ...
  useEffect(()=>{
    const sync=()=>{
      ...
      setWorkspace(ws)
      const nativeTitle=document.querySelector('.workspace > main .page-header h1')?.textContent?.trim()
      const scenarioOpen=document.body.classList.contains('scenario-analysis-open')
      setActive(nativeTitle==='Dashboard'&&!scenarioOpen)
    ...
    const delayed=()=>{sync();setTimeout(sync,0);setTimeout(sync,80)}
    delayed()
    const observer=new MutationObserver(delayed)
    observer.observe(document.body,{childList:true,subtree:true,attributes:true,characterData:true,attributeFilter:['class','value']})
    document.addEventListener('change',delayed,true)
    document.addEventListener('click',delayed,true)
    return()=>{observer.disconnect();document.removeEventListener('change',delayed,true);document.removeEventListener('click',delayed,true)}
  },[])
  ...
  useEffect(()=>{
    if(!active||!supabase||!depositanteId){setHistory([]);setRows([]);setDepositante(null);setSelectedPlanningId('');return}
    let cancelled=false
    Promise.all([
