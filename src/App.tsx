...
import { simulate, type Decision, type Decisions, type Depositor, type ForecastRow } from './engine/simulation'
...
const pageMeta: Record<Page, { title: string; description: string }> = {
  ...
  simulation: { title: 'Nova simulação', description: 'Forecast, capacidade, headcount e decisões operacionais por dia.' },
...
export default function App() {
  ...
  const results = useMemo(() => simulate(rows, activeDepositor, new Set<string>(), decisions), [rows, activeDepositor, decisions])
  ...
  useEffect(() => {
    ...
    const { data } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      else { setProfile(null); setDepositors([]); setSelectedId(''); setRows([]); setDecisions({}) }
    })
  ...
  function downloadTemplate() {
    const sample = [
      { Data: '2026-11-23', Forecast: 3200 }, { Data: '2026-11-24', Forecast: 4100 },
      { Data: '2026-11-25', Forecast: 5200 }, { Data: '2026-11-26', Forecast: 6100 },
      { Data: '2026-11-27', Forecast: 7200 },
    ]
    const ws = XLSX.utils.json_to_sheet(sample); ws['!cols'] = [{ wch: 14 }, { wch: 14 }]
  ...
  async function deleteDepositor() {
    ...
    const { data: { user } } = await supabase.auth.getUser(); if (user) await loadUserData(user.id)
  }
  ...
  return <div className={`shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''} ${mobileMenuOpen ? 'mobile-menu-open' : ''}`}>
    ...
    <aside className="sidebar">
      <div className="sidebar-head">
        ...
        <button className="icon-button collapse-button" onClick={() => setSidebarCollapsed(v => !v)}><MaterialIcon name={sidebarCollapsed ? 'menu' : 'menu_open'} size={20}/></button>
      ...
      <div className="user-card"><div className="user-avatar">{(profile.nome ?? 'U').slice(0,1).toUpperCase()}</div><div className="user-copy"><span>{profile.nome ?? 'Usuário'}</span><small>{isAdmin ? 'Administrador' : 'Usuário'}</small></div><button className="icon-button signout-button" onClick={signOut}><MaterialIcon name="logout" size={18}/></button></div>
    </aside>

    <div className="workspace">
      <div className="topbar"><div className="topbar-left"><button className="icon-button mobile-menu-button" onClick={() => setMobileMenuOpen(true)}><MaterialIcon name="menu" size={21}/></button><span className="current-page">{meta.title}</span></div><div className="topbar-right">{depositors.length > 0 && <label className="topbar-select"><select value={selectedId} onChange={e => setSelectedId(e.target.value)}>{depositors.filter(d => d.ativo).map(d => <option key={d.id} value={d.id}>{d.nome}</option>)}</select><MaterialIcon name="expand_more" size={17}/></label>}<div className="profile-chip"><div className="mini-avatar">{(profile.nome ?? 'U').slice(0,1).toUpperCase()}</div><span>{profile.nome ?? 'Usuário'}</span></div></div></div>
      ...
      <main>
        ...
        {page === 'dashboard' && <>
          ...
          <section className="kpi-strip"><div><span>Forecast total</span><strong>{results.length ? total.toLocaleString('pt-BR') : '—'}</strong><small>{results.length ? `${results.length} dias` : 'Sem forecast'}</small></div><div className={backlog > 0 ? 'kpi-warning' : ''}><span>Backlog final</span><strong>{results.length ? backlog.toLocaleString('pt-BR') : '—'}</strong><small>{results.length ? `${backlogDays} dias com saldo` : 'Sem simulação'}</small></div><div><span>Pico de HC</span><strong>{results.length ? peakHC : '—'}</strong><small>{results.length ? `${peakCheckouts} checkouts no pico` : 'Sem simulação'}</small></div><div><span>Ações pendentes</span><strong>{results.length ? pendingDecisions : '—'}</strong><small>{results.length ? 'aguardando decisão' : 'Sem simulação'}</small></div></section>
          <section className="content-grid"><article className="section-panel"><div className="section-heading"><div><h2>Limitantes ativos</h2><p>Parâmetros que substituem os números antes escondidos nas fórmulas.</p></div><button className="link-action inline" onClick={() => navigate('parameters')}>Editar <MaterialIcon name="chevron_right" size={17}/></button></div><dl className="details-list"><div><dt>Checkouts mínimos · dia útil</dt><dd>{activeDepositor.checkouts_minimos_dia_util}</dd></div><div><dt>Checkouts máximos · dia útil</dt><dd>{activeDepositor.checkouts_maximos}</dd></div><div><dt>Checkouts máximos · fim de semana</dt><dd>{activeDepositor.checkouts_maximos_fim_semana}</dd></div><div><dt>HE máxima</dt><dd>{activeDepositor.horas_extra_max_dia_util}h</dd></div><div><dt>Turnos máximos</dt><dd>{activeDepositor.turnos_maximos}</dd></div></dl></article><article className="section-panel"><div className="section-heading"><div><h2>Composição de HC</h2><p>Consumo de pessoas exposto pelo planejador.</p></div></div><dl className="details-list"><div><dt>Pessoas por checkout</dt><dd>{activeDepositor.pessoas_por_checkout}</dd></div><div><dt>Apoio fixo atual</dt><dd>{activeDepositor.pessoas_separando + activeDepositor.pessoas_embalando + activeDepositor.pessoas_embalagem_caixa + activeDepositor.pessoas_roteirizando + (activeDepositor.tipo_colmeia === 'Móvel' ? 0 : activeDepositor.pessoas_ressuprindo)}</dd></div><div><dt>HC máximo</dt><dd>{activeDepositor.hc_maximo}</dd></div></dl></article></section>
        </>}

        {page === 'simulation' && <>
          <section className="simulation-summary"><div><span>Operação</span><b>{activeName}</b></div><div><span>Forecast</span><b>{results.length ? total.toLocaleString('pt-BR') : '—'}</b></div><div><span>Backlog final</span><b className={backlog > 0 ? 'text-warning' : ''}>{results.length ? backlog.toLocaleString('pt-BR') : '—'}</b></div><div><span>Decisões pendentes</span><b>{results.length ? pendingDecisions : '—'}</b></div></section>
          <section className="section-panel upload-panel"><div className="section-heading"><div><h2>Arquivo de forecast</h2><p>Use Data + Forecast. O motor sugere ações, mas a decisão é sua.</p></div></div><label className={`dropzone ${file ? 'has-file' : ''}`}><input type="file" accept=".xlsx,.xls,.csv" onChange={e => e.target.files?.[0] && onFile(e.target.files[0])}/><MaterialIcon name={file ? 'check_circle' : 'upload_file'} size={24}/><div><b>{file || 'Selecionar arquivo'}</b><small>{file ? `${rows.length} registros válidos` : 'Excel ou CSV · processamento local'}</small></div><span className="secondary-action fake-button">{file ? 'Trocar' : 'Procurar'}</span></label></section>
          {results.length > 0 ? <section className="section-panel table-panel planning-table"><div className="section-heading table-heading"><div><h2>Planejamento diário</h2><p>Ações pendentes são cenários provisórios até você aceitar ou rejeitar.</p></div><span className={`plain-status ${pendingDecisions ? 'warning' : 'success'}`}><MaterialIcon name={pendingDecisions ? 'warning' : 'check_circle'} size={17}/>{pendingDecisions ? `${pendingDecisions} pendente(s)` : 'Decisões revisadas'}</span></div><div className="table-wrap"><table><thead><tr><th>Data</th><th>Tipo</th><th>Forecast</th><th>Backlog ant.</th><th>Necessidade</th><th>Cap. base</th><th>Checkouts</th><th>HC checkout</th><th>HC apoio</th><th>HC total</th><th>Cap. planejada</th><th>Ação sugerida / decisão</th><th>Backlog final</th></tr></thead><tbody>{results.map(r => <tr key={r.data} className={r.backlogFinal > 0 ? 'row-attention' : ''}><td className="date-cell">{new Date(r.data + 'T12:00:00').toLocaleDateString('pt-BR')}</td><td>{r.tipoDia}</td><td>{r.forecast.toLocaleString('pt-BR')}</td><td>{r.backlogAnterior.toLocaleString('pt-BR')}</td><td>{r.producaoNecessaria.toLocaleString('pt-BR')}</td><td>{r.capacidadeBase.toLocaleString('pt-BR')}</td><td>{r.checkouts}</td><td>{r.hcCheckout}</td><td>{r.hcApoio}</td><td><b>{r.hcTotal}</b></td><td>{r.capacidade.toLocaleString('pt-BR')}</td><td className="decision-cell"><div className="suggestion-title">{r.acaoSugerida}</div>{r.decisao === 'base' ? <span className="decision-status base">Operação base</span> : <><span className={`decision-status ${r.decisao}`}>{r.decisao === 'pending' ? 'Sugestão pendente' : r.decisao === 'accepted' ? 'Aceita' : 'Rejeitada'}</span><div className="decision-actions"><button className={r.decisao === 'accepted' ? 'decision-btn selected' : 'decision-btn'} onClick={() => setDecision(r.data,'accepted')}>Aceitar</button><button className={r.decisao === 'rejected' ? 'decision-btn selected reject' : 'decision-btn reject'} onClick={() => setDecision(r.data,'rejected')}>Rejeitar</button>{r.decisao !== 'pending' && <button className="decision-btn clear" onClick={() => clearDecision(r.data)}>Rever</button>}</div></>}</td><td className={r.backlogFinal > 0 ? 'backlog-value' : ''}>{r.backlogFinal.toLocaleString('pt-BR')}</td></tr>)}</tbody></table></div></section> : <section className="empty-panel"><MaterialIcon name="table_chart" size={26}/><div><h2>Planejamento não calculado</h2><p>Importe o forecast para gerar a tabela diária.</p></div></section>}
        </>}
