...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      else { setProfile(null); setDepositors([]); setSelectedId(''); setRows([]); setDecisions({}); setTariffs(emptyTariffs); setHolidays(new Set()) }
    })
  ...
  async function loadUserData(userId: string) {
    if (!supabase) return
    const [{ data: p }, { data: deps, error }, { data: rateRows, error: rateError }, { data: holidayRows, error: holidayError }] = await Promise.all([
  ...
  async function onFile(f:File){
    ...
    const parsed=data.map(x=>{const raw=x['Data'];const d=raw instanceof Date?raw.toISOString().slice(0,10):String(raw??'').slice(0,10);const fc=Number(x['Forecast']??x['Forecast Total Pedido Normal']??0);return{data:d,forecast:fc}}).filter(x=>/^\d{4}-\d{2}-\d{2}$/.test(x.data)&&Number.isFinite(x.forecast)&&x.forecast>=0)
    setRows(parsed);setFile(f.name);setDecisions({})
  }
  function downloadTemplate(){const sample=[{Data:'2026-11-23',Forecast:3200},{Data:'2026-11-24',Forecast:4100},{Data:'2026-11-25',Forecast:5200},{Data:'2026-11-26',Forecast:6100},{Data:'2026-11-27',Forecast:7200}];const ws=XLSX.utils.json_to_sheet(sample);ws['!cols']=[{wch:14},{wch:14}];const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Forecast');XLSX.writeFile(wb,'MODELO_FORECAST.xlsx')}
  ...
  async function saveDepositor(){if(!supabase||!isAdmin)return;setLoading(true);setDataMessage('');const payload={...form,updated_at:new Date().toISOString()} as any;let error;if(selectedId)({error}=await supabase.from('depositantes').update(payload).eq('id',selectedId));else({error}=await supabase.from('depositantes').insert(payload));setDataMessage(error?error.message:'Parâmetros salvos com sucesso.');const{data:{user}}=await supabase.auth.getUser();if(!error&&user)await loadUserData(user.id);setLoading(false)}
