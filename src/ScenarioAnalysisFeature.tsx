...
import { analyzeScenario } from './engine/scenario'
...
export default function ScenarioAnalysisFeature(){
  ...
  const analysis=useMemo(()=>activeDepositor?analyzeScenario(rows,activeDepositor,holidays):null,[rows,activeDepositor,holidays])
  ...
  const navButton=navTarget?createPortal(
    <button data-scenario-nav="true" className={open?'active scenario-nav-button':'scenario-nav-button'} onClick={()=>setOpen(true)} title="Análise de cenário">
      <span className="material-symbols-rounded material-icon" aria-hidden="true">difference</span>
  ...
  const page=open&&workspaceTarget?createPortal(
    <div className="scenario-analysis-page">
      ...
      <section className="scenario-control-bar">
        ...
        <div className="scenario-current-context">
          <span>Estrutura cadastrada</span>
          <b>{activeDepositor?`${activeDepositor.checkouts_atuais} checkouts · ${number(activeDepositor.capacidade_checkout_dia)} pedidos/checkout`:'—'}</b>
        </div>
      </section>
      ...
      {summary&&rows.length>0?<>
        ...
        <section className="scenario-kpis">
          ...
          <div><span>Utilização máxima atual</span><strong>{summary.maxUtilization.toLocaleString('pt-BR',{maximumFractionDigits:1})}%</strong><small>forecast ÷ capacidade instalada</small></div>
          ...
          <div><span>Dias fora da escala</span><strong>{summary.daysOutsideSchedule}</strong><small>com forecast maior que zero</small></div>
        </section>

        <section className="section-panel table-panel scenario-table-panel">
          <div className="section-heading table-heading"><div><h2>Dimensionamento diário</h2><p>O necessário não é limitado pelos checkouts existentes: ele mostra a estrutura teórica para atender a demanda diária sem backlog.</p></div></div>
          <div className="table-wrap"><table className="scenario-table"><thead><tr><th>Data</th><th>Forecast</th><th>Cap. atual</th><th>Utilização</th><th>Checkouts atuais</th><th>Checkouts necessários</th><th>Gap checkouts</th><th>HC atual</th><th>HC necessário</th><th>Gap HC</th><th>Limite cadastrado</th><th>Leitura</th></tr></thead><tbody>{days.map(row=><tr key={row.data} className={row.status==='Expansão estrutural'?'scenario-critical':row.gapCheckouts>0?'scenario-attention':''}><td className="date-cell">{date(row.data)}<small>{row.tipoDia}{row.dentroEscala?'':' · fora da escala'}</small></td><td>{number(row.forecast)}</td><td>{number(row.capacidadeAtual)}</td><td>{row.utilizacaoAtual==null?'—':`${row.utilizacaoAtual.toLocaleString('pt-BR',{maximumFractionDigits:1})}%`}</td><td>{row.checkoutsAtuais}</td><td><b>{row.checkoutsNecessarios}</b></td><td className={row.gapCheckouts>0?'scenario-gap-value':''}>{row.gapCheckouts>0?`+${row.gapCheckouts}`:'—'}</td><td>{row.hcAtual}</td><td>{row.hcNecessario}</td><td className={row.gapHC>0?'scenario-gap-value':''}>{row.gapHC>0?`+${row.gapHC}`:'—'}</td><td>{row.limiteCheckouts}</td><td><span className={`scenario-status ${row.status.toLowerCase().replaceAll(' ','-').normalize('NFD').replace(/[\u0300-\u036f]/g,'')}`}>{row.status}</span></td></tr>)}</tbody></table></div>
        </section>
      </>:<section className="empty-panel scenario-empty"><span className="material-symbols-rounded material-icon">query_stats</span><div><h2>Importe o forecast para comparar os cenários</h2><p>A estrutura atual será comparada com a necessidade de checkouts, capacidade e headcount de cada dia.</p></div></section>}
